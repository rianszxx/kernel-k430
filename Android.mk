...
ifeq ($(PRODUCT_SUPPORT_EXFAT), y)
	...
	@sh tuxera_update.sh --target target/lg.d/mobile-mt6753 --use-cache --latest --max-cache-entries 2 --source-dir $(ANDROID_BUILD_TOP)/kernel --output-dir $(KERNEL_OUT) -a --user lg-mobile --pass AumlTsj0ou
	...
	perl $(ANDROID_BUILD_TOP)/kernel/scripts/sign-file sha1 $(KERNEL_OUT)/signing_key.priv $(KERNEL_OUT)/signing_key.x509 $(KERNEL_MODULES_OUT)/lib/modules/texfat.ko
...
$(BUILT_KERNEL_TARGET): $(KERNEL_ZIMAGE_OUT) $(TARGET_KERNEL_CONFIG) $(KERNEL_HEADERS_INSTALL) $(LOCAL_PATH)/Android.mk | $(HOST_OUT_EXECUTABLES)/mkimage$(HOST_EXECUTABLE_SUFFIX)
	$(hide) $(HOST_OUT_EXECUTABLES)/mkimage$(HOST_EXECUTABLE_SUFFIX) $< KERNEL 0xffffffff > $@

else
$(BUILT_KERNEL_TARGET): $(KERNEL_ZIMAGE_OUT) $(TARGET_KERNEL_CONFIG) $(KERNEL_HEADERS_INSTALL) $(LOCAL_PATH)/Android.mk | $(ACP)
	$(copy-file-to-target)
...
$(KERNEL_HEADERS_INSTALL): $(KERNEL_OUT)
	$(hide) if [ ! -z "$(KERNEL_HEADER_DEFCONFIG)" ]; then \
			...
			$(MAKE) -C $(KERNEL_DIR) $(KERNEL_MAKE_OPTION) $(KERNEL_HEADER_DEFCONFIG); \
	...
	$(hide) if [ ! -z "$(KERNEL_CONFIG_OVERRIDE)" ]; then \
			...
			$(MAKE) $(KERNEL_DIR) $(KERNEL_MAKE_OPTION) oldconfig; fi
