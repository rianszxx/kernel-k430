...
void arch_reset(char mode, const char *cmd)
{
	...
	volatile u32 *lge_boot_magic_number = (volatile u32*)(LGE_RAM_CONSOLE_MEM_VBASE + LGE_BSP_RAM_CONSOLE_SIZE - 32); /* jjm_sram */
	...
	if (cmd && !strcmp(cmd, "charger")) {
		...
		lge_save_boot_reason(LGE_BOOT_FOTA_LCD_OFF, 0, 0);
		reboot = 1;
	}
	else if (cmd && !strcmp(cmd, "FOTA OUT LCD off")) {
		lge_save_boot_reason(LGE_BOOT_FOTA_OUT_LCD_OFF, 0, 0);
		reboot = 1;
	}
	else if (cmd && !strcmp(cmd, "LCD off")) {
		lge_save_boot_reason(LGE_BOOT_LCD_OFF, 0, 0);
		reboot = 1;
	}
#endif
#endif
