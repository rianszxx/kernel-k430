...
static long APDS9930_enable_als(struct i2c_client *client, int enable)
{
	...
	if (res <= 0)
	 ...
	 APS_ERR("APDS9930_CMM_ENABLE als value = %x\n",databuf[0]);
...
static long APDS9930_enable_ps(struct i2c_client *client, int enable)
{
	...
	if (res <= 0)
	 ...
	 APS_ERR("APDS9930_CMM_ENABLE ps value = %x\n",databuf[0]);
	...
	if (enable) {
		...
		set_psensor_threshold(client,750,900);
		/*fix bug */
	}
	return 0;
...
static void APDS9930_irq_work(struct work_struct *work)
{
	...
	unsigned int als_low_threshold, als_high_threshold;
...
/*signal interrupt function add*/
		if (ps_state) {  //far
			set_psensor_threshold(obj->client,0,obj->local_hw->far_offset);

		} else {
			set_psensor_threshold(obj->client,obj->local_hw->near_offset,1023);
		}
	...
	if(res == ALS_INT || res == PS_ALS_INT){
		...
		adata = APDS9930_get_als_value(obj, obj->als);
		...
		if(atomic_read(&driver_suspend_flag) == 1){
			...
			if(als_ch0 <= ALS_LUX_THRESHOLD){
				APS_ERR("als_data(%d)<THRESHOLD(%d), Change bright to dark\n",adata,ALS_LUX_THRESHOLD);
				set_asensor_threshold(obj->client,0,ALS_LUX_THRESHOLD);
			}else{
				APS_ERR("als_data(%d)>THRESHOLD(%d), Change dark to bright\n",adata,ALS_LUX_THRESHOLD);
				set_asensor_threshold(obj->client,ALS_LUX_THRESHOLD, 0xFFFF);
			}
...
static long APDS9930_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ALSPS_SET_PS_THRESHOLD:
		...
		set_psensor_threshold(obj->client, obj->local_hw->far_offset,obj->local_hw->near_offset);
