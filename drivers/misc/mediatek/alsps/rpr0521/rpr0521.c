...
#define SET_PS_PULSE_200US	(0 << 5)
#define SET_PS_PULSE_330US	(1 << 5)

#define SET_PS_MODE_NORMAL	(0 << 4)
#define SET_PS_MODE_TWICE	(1 << 4)
...
#define PS_INFRARED_LEVEL_LOW		(0 << 6)
#define PS_INFRARED_LEVEL_HIGH		(1 << 6)
#define PS_INFRARED_LEVEL_TOO_HIGH	(3 << 6)

#define INTR_PERSIST         (2)
#define INIT_PS_GAIN_X1      (0 << 4)
#define INIT_PS_GAIN_X2      (1 << 4)
#define INIT_PS_GAIN_X4      (2 << 4)
#define PWRON_PS             (6)
#define PWRON_ALS            (7)
...
#define INIT_MODE_CONTROL         (SET_PS_PULSE_200US | SET_PS_MODE_NORMAL | SET_ALS_100_PS_100MS)
#define INIT_ALSPSMODE_CONTROL    (LEDCURRENT_100MA)
//#define INIT_ALSPSMODE_CONTROL    (LEDCURRENT_50MA | ALSGAIN_X1X1)//with als gain
#define INIT_PSMODE_CONTROL       (PS_INFRARED_LEVEL_LOW | INIT_PS_GAIN_X4 | INTR_PERSIST)
...
static int Target_Pdata = 38 /*12*/ /*40*/; /* parameter for taget pdata setting */
static int NearToFar = 11 /*4*/ /*20*/; /* parameter for far detection */
...
static int rpr0521_remove(void);
...
//==========================================================
static int rpr0521_setup_eint ( void )
{
	APS_FUN ();
	int ret = 0;
	u32 ints[2] = { 0, 0 };
	
	g_rpr0521_ptr->irq_node = of_find_compatible_node(NULL, NULL, "mediatek, als-eint");
	if (g_rpr0521_ptr->irq_node) {
		of_property_read_u32_array(g_rpr0521_ptr->irq_node, "debounce", ints, ARRAY_SIZE(ints));
		gpio_set_debounce(ints[0], ints[1]);
		APS_LOG("ints[0] = %d, ints[1] = %d!!\n", ints[0], ints[1]);

		g_rpr0521_ptr->irq = irq_of_parse_and_map(g_rpr0521_ptr->irq_node, 0);
		if (g_rpr0521_ptr->irq < 0) {
			APS_ERR("irq_of_parse_and_map IRQ LINE NOT AVAILABLE!.");
			return -1;
		}

		ret = request_irq(g_rpr0521_ptr->irq, rpr0521_eint_handler, IRQF_TRIGGER_FALLING,"ALS-eint", NULL);
		if (ret > 0) {
			APS_ERR("IRQ LINE NOT AVAILABLE!!\n");
			return -EINVAL;
		}
//		enable_irq(g_rpr0521_ptr->irq);	
//		APS_LOG("enable_irq in rpr0521_setup_eint function\n");
	} else {
...
static long rpr0521_initialize ( struct i2c_client *client  )
{
	...
	rpr0521_set_control ( client, INIT_MODE_CONTROL );
	rpr0521_set_alsps_control ( client, INIT_ALSPSMODE_CONTROL);
...
{
	rpr0521_eint_func();
	disable_irq_nosync(g_rpr0521_ptr->irq);
...
#if defined(CONFIG_OF)
	enable_irq(obj->irq);
	APS_LOG("enable_irq in rpr0521_eint_work function\n");
#else
	mt_eint_unmask ( CUST_EINT_PROXIMITY_NUM );
#endif
...
static ssize_t rpr0521_store_near_to_far( struct device_driver *dev, char *buf, size_t count )
{
	struct i2c_client *client = rpr0521_i2c_client;
	struct rpr0521_priv *obj = i2c_get_clientdata ( client );
	unsigned long val = simple_strtoul ( buf, NULL, 10 );
...
static DRIVER_ATTR ( near_to_far, S_IWUSR | S_IRUGO | S_IWGRP | S_IRGRP | S_IROTH, rpr0521_show_near_to_far, rpr0521_store_near_to_far );
	...
	switch ( command )
	{
		...
		case SENSOR_GET_DATA:
			...
			else
			{
				...
				sensor_data->values[0] = (int)obj->ps_status;
				sensor_data->value_divide = 1;
				sensor_data->status = SENSOR_STATUS_ACCURACY_MEDIUM;
				APS_LOG("values[0] %d==%d\n",sensor_data->values[0],obj->ps_status);
			}
			break;
...
static struct i2c_driver rpr0521_i2c_driver = { 
	...
	.suspend = rpr0521_i2c_suspend,
	.resume = rpr0521_i2c_resume,
	.detect = rpr0521_i2c_detect,
	.id_table = rpr0521_i2c_id,
	.driver = {
		.name = RPR0521_DEV_NAME,
#ifdef CONFIG_OF
...
static struct i2c_board_info __initdata i2c_RPR0521 = { I2C_BOARD_INFO ( RPR0521_DEV_NAME, RPR0521_I2CADDR ) };
/*
...
{ 
	...
	//rpr0521_main_power ( hw, 1 );
	//msleep ( 9 );
	i2c_register_board_info(hw->i2c_num, &i2c_RPR0521, 1);
	/* Add RPR0521 as I2C driver */
	if ( i2c_add_driver ( &rpr0521_i2c_driver ) )
	{
		APS_ERR ( "failed to add i2c driver\n" );
