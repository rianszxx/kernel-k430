...
void mtk_gpu_dvfs_commit(unsigned long ui32NewFreqID, GED_DVFS_COMMIT_TYPE eCommitType, int* pbCommited)
{
    ...
    if(loading > GPU_DVFS_THRESHOLD){
        ret = mt_gpufreq_target(ui32NewFreqID);
    }else{
        if(ui32NewFreqID >= mt_gpufreq_get_dvfs_table_num()-1 ){
            ret = mt_gpufreq_target(mt_gpufreq_get_dvfs_table_num()-1);
        }else{
            ret = mt_gpufreq_target(ui32NewFreqID+1);
        }
    }
