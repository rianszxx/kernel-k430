...
#define LEDS_DEBUG(format, args...) do { \
	if (debug_enable_led_hal) {	\
		pr_debug("[LED_MT6735]"format, ##args);\
	} \
} while (0)
...
int mt_mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
	...
	case MT65XX_LED_MODE_CUST_LCM:
...
#ifdef CONFIG_LGE_LCD_OFF_DIMMING
		{
			...
			if ((lge_boot_reason == LGE_BOOT_FOTA_LCD_OFF
				|| lge_boot_reason == LGE_BOOT_FOTA_OUT_LCD_OFF
				|| lge_boot_reason == LGE_BOOT_LCD_OFF)
				&& !fb_blank_called) {
...
#if defined(TARGET_MT6582_Y90) || defined(TARGET_MT6582_Y70) || defined(TARGET_MT6753_K7) || defined(TARGET_MT6582_B2L) || defined(TARGET_MT6582_P1S3G) /* LGE_BSP_COMMON sangcheol.seo@lge.com 140520 : LED */
typedef struct {
...
#if defined(TARGET_MT6582_Y90)|| defined(TARGET_MT6753_K7)
...
int mt_mt65xx_breath_mode(unsigned int patternid)
{
	...
	LEDS_DEBUG(" ch1_step = 0x%x, dim1_duty = %d, dim1_fsel = %d, breath1_trf_sel = %d, breath1_ton_sel = %d, breath1_toff_sel = %d\n",\
	...
	pmic_set_register_value(PMIC_ISINK_BREATH0_TR1_SEL, led_scene_setting[patternid].isink1_breath1_trf_sel);
