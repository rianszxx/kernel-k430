...
#ifdef MT6735
//#define POWER_NONE_MACRO MT65XX_POWER_NONE
#define POWER_NONE_MACRO -1//temp define
#endif
...
#define BMA255_DEV_NAME        "BMA255_misc"
...
static int bma255_init_flag = BMA255_INIT_FAIL;
static struct sensor_init_info bma255_init_info = {
	.name = BMA255_DEV_NAME,
...
static ssize_t bma255_fast_calibration_x_store(struct device_driver *ddri, char *buf, size_t count)
{
	...
	if (error = kstrtoint(buf, 10, &data))
		return error;
...
static int bma255_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	bma255_init_flag=BMA255_INIT_SUCC;
	SENSOR_LOG("%s: OK\n", __func__);
	return 0;
...
exit_kfree:
	kfree(obj);
exit:
	bma255_init_flag=BMA255_INIT_FAIL; 
	SENSOR_ERR("err = %d\n", err);
	return err;
}
...
static int bma255_i2c_remove(struct i2c_client *client)
{
	...
	{
		SENSOR_ERR("bma255_delete_attr fail: %d\n", err);
	}
#else
...
static int  bma255_local_init(void)
{
	...
	if (BMA255_INIT_FAIL == bma255_init_flag)
		return -1;

	SENSOR_LOG("i2c_number = %d / bma255_local_init success\n\n", hw->i2c_num);
	return 0;
}

static int  bma255_local_remove(void)
{
    SENSOR_FUN();
    i2c_del_driver(&bma255_i2c_driver);
    return 0;
