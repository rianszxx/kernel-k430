...
void __iomem *lge_ramconsole_vbase;
unsigned long LGE_BSP_MEM_MAX_PHY_ADDR;
unsigned long LGE_RAM_CONSOLE_MEM_VBASE;
#endif
...
#if defined (CONFIG_LGE_HANDLE_PANIC) || defined (CONFIG_LGE_HIDDEN_RESET)
#if defined (CONFIG_LGE_ONE_BIN_MEMORY)
	LGE_BSP_MEM_MAX_PHY_ADDR = (LGE_BSP_MEM_BASE_ADDR + phone_dram_sz);
	pr_err("[ONE_BIN_MEMORY] LGE_BSP_MEM_MAX_PHY_ADDR : 0x%x\n", LGE_BSP_MEM_MAX_PHY_ADDR);
#endif

	lge_ramconsole_vbase = ioremap(LGE_BSP_RAM_CONSOLE_PHY_ADDR, LGE_BSP_RAM_CONSOLE_SIZE);
	LGE_RAM_CONSOLE_MEM_VBASE = (unsigned long)lge_ramconsole_vbase;
	pr_err("LGE_RAM_CONSOLE_MEM_VBASE: 0x%x\n", LGE_RAM_CONSOLE_MEM_VBASE);
#endif
