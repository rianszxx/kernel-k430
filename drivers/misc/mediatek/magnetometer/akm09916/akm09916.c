...
static int akm09916_local_init(struct platform_device *pdev);
static int akm09916_remove(struct platform_device *pdev);
...
/*
	...
	{        
		...
		{
			if(!hwPowerOn(hw->power_id, hw->power_vol, "akm09916")) 
			{
		...
		{
			if(!hwPowerDown(hw->power_id, "akm09916")) 
			{
...
static void akm09916_early_suspend(struct early_suspend *h) 
{
	...
	if ((err = AKECS_SetMode(AK09916_MODE_POWERDOWN)) < 0) {
		AKMDBG("%s:%d Error.\n", __FUNCTION__, __LINE__);
		return;
	}
...
static void akm09916_late_resume(struct early_suspend *h)
{
	...
	if ((err = AKECS_SetMode(AK09916_MODE_SNG_MEASURE)) < 0) {
		AKMDBG("%s:%d Error.\n", __FUNCTION__, __LINE__);
		return;
		}
}
...
static int akm09916_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	this_client = new_client;	
     ...
     printk(KERN_ERR " AKM09916 akm09916_probe: befor init prob \n");
	/* Check connection */
	...
	if((err = akm09916_create_attr(&akm09916_init_info.platform_diver_addr->driver)))
	{
...
static int akm09916_i2c_remove(struct i2c_client *client)
{
	...
	if((err = akm09916_delete_attr(&akm09916_init_info.platform_diver_addr->driver)))
	{
