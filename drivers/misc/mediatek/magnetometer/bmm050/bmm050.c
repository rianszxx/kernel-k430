...
static int bmm050_init_flag = BMM050_INIT_FAIL;
...
static int ECS_SaveData(int buf[CALIBRATION_DATA_SIZE])
{
	...
	{
		SENSOR_LOG("Get daemon data: %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d!\n",
...
#if defined(BMC050_M4G) || defined(BMC050_VRV)
		SENSOR_LOG("Get m4g data: %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d!\n",
		...
		SENSOR_LOG("Get flip data: %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d!\n",
...
static void bmm050_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	SENSOR_LOG("bmm050_setPowerMode enable : %d\n",enable);
	if (enable == FALSE)
	{
		if (bmm050api_set_functional_state(BMM050_SUSPEND_MODE) != 0)
		{
			SENSOR_ERR("fail to suspend sensor");
			return;
		}
...
static ssize_t show_trace_value(struct device_driver *ddri, char *buf)
{
	...
	if(NULL == obj)
	{
		SENSOR_ERR( "bmm050_i2c_data is null!!\n");
		return 0;
	}
...
static int bmm050_open(struct inode *inode, struct file *file)
{
	...
	{
		SENSOR_LOG("Open device node:bmm050\n");
	}
...
static int bmm050_release(struct inode *inode, struct file *file)
{
	...
	if(atomic_read(&obj->trace) & MMC_CTR_DEBUG)
	{
		SENSOR_LOG("Release device node:bmm050\n");
	}
	return 0;
}
/*----------------------------------------------------------------------------*/
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			else
			{
				...
				if(atomic_read(&data->trace) & MMC_HWM_DEBUG)
				{
					SENSOR_LOG("Hwm get m-sensor data: %d, %d, %d. divide %d, status %d!\n",
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			else
			{
				...
				if(atomic_read(&data->trace) & MMC_HWM_DEBUG)
				{
					SENSOR_LOG("Hwm get o-sensor data: %d, %d, %d. divide %d, status %d!\n",
			...
			break;
		default:
			SENSOR_ERR( "osensor operate function no this parameter %d!\n", command);
			err = -1;
			break;
	}
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
					...
					
			{
				...
				if(atomic_read(&data->trace) & MMC_HWM_DEBUG)
				{
					SENSOR_LOG("Hwm get m4g data: %d, %d, %d. divide %d, status %d!\n",
			...
			break;
		default:
			SENSOR_ERR( "m4g operate function no this parameter %d!\n", command);
			err = -1;
			break;
	}
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			else
			{
				...
				if(atomic_read(&data->trace) & MMC_HWM_DEBUG)
				{
					SENSOR_LOG("Hwm get rotation vector data: %d, %d, %d. divide %d, status %d!\n",
			...
			break;
		default:
			SENSOR_ERR( "rotation vector operate function no this parameter %d!\n", command);
			err = -1;
			break;
	}
		...
		void* buff_out, int size_out, int* actualout)
{
	int err = 0;
	int value;
	struct hwm_sensor_data* gest_flip_data;
#if DEBUG
	struct i2c_client *client = this_client;
	struct bmm050_i2c_data *data = i2c_get_clientdata(client);
#endif
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			{
				gest_flip_data = (struct hwm_sensor_data *)buff_out;
				mutex_lock(&sensor_data_mutex);
				...
				if(atomic_read(&data->trace) & MMC_HWM_DEBUG)
				{
					SENSOR_LOG("Hwm get gesture filp data: %d, %d, %d. divide %d, status %d!\n",
...
static void bmm050_early_suspend(struct early_suspend *h)
{
	...
	SENSOR_LOG("bmm050_early_suspend==================================\n");
...
static int bmm050_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	{
		SENSOR_ERR("fail to initialize client");
	...
	{
		SENSOR_ERR("bmm050_device register failed\n");
...
#endif
	bmm050_init_flag=BMM050_INIT_SUCC; 
	SENSOR_LOG("%s: OK\n", __func__);
	return 0;
	...
	kfree(data);
	exit:
		bmm050_init_flag=BMM050_INIT_FAIL;
	SENSOR_ERR( "%s: err = %d\n", __func__, err);
	return err;
}
/*----------------------------------------------------------------------------*/
static int bmm050_i2c_remove(struct i2c_client *client)
{
	...
	{
		SENSOR_ERR( "bmm050_delete_attr fail");
	}
...
static int  bmm050_local_init(void)
{
	...
	if(BMM050_INIT_FAIL == bmm050_init_flag)
	{
	   SENSOR_ERR("bmm050_local_init add driver error\n");
	   return -1;
	}
