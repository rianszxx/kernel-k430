...
/* set freq (UP/DOWN) */
	...
	/* MTK Guide : Print log different over Freq 600Mhz */
	if (abs(target_khz - cur_khz) > 600000) {
...
#if defined(CONFIG_ARCH_MT6753) && !defined(LIMIT_CORE_FOR_5A_THROTTLE)
...
#ifdef LIMIT_CORE_FOR_5A_THROTTLE
		if (pmic_5A_throttle_on)
			hps_set_cpu_num_limit(LIMIT_POWER_SERV, PMIC_5A_THRO_MAX_CPU_CORE_NUM, 0);
		else
			hps_set_cpu_num_limit(LIMIT_POWER_SERV, num_possible_cpus(), 0);
#endif
