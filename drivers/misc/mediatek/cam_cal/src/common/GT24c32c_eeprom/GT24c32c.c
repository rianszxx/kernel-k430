...
int iReadDataFromGT24c32c(unsigned int  ui4_offset, unsigned int  ui4_length,
...
*/
	...
	case CAM_CALIOC_G_READ:
		...
		else if (ptempbuf->u4Offset == 0x0024C32c) {
			*(u32 *)pWorkingBuff = 0x0124C32c;
...
#else
#if 0
		if (ptempbuf->u4Offset == 0x0A04C32C) {
			*(u32 *)pWorkingBuff = 0x1A04C32C;
#endif
			...
			else if( ((ptempbuf->u4Offset & 0x000FFFFF) == 0x0004C32C) && (EEPROM_Address[0]==0x0) )
			{
			...
			else if( ((ptempbuf->u4Offset & 0x000FFFFF) == 0x0004C32C) && (ResetCheck == 0xABCDFEDC) )
			{
				*(u32 *)pWorkingBuff = (ptempbuf->u4Offset | 0x10000000);
				...
				while (0 < readTryagain) {
					i4RetValue =  iReadDataFromGT24c32c(ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff);
...
static unsigned short force[] = {EEPROM_I2C_GROUP_ID, GT24C32C_DEVICE_ID, I2C_CLIENT_END,
	I2C_CLIENT_END};
#else
/* static unsigned short force[] = {IMG_SENSOR_I2C_GROUP_ID, GT24C32C_DEVICE_ID, I2C_CLIENT_END, I2C_CLIENT_END}; */
#endif
...
static int EEPROM_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	g_pstI2Cclient->addr = GT24C32C_DEVICE_ID >> 1;
	spin_unlock(&g_EEPROMLock); /* for SMP */
...
#if defined( COMMON_CAM_CAL_DRV)
EXPORT_SYMBOL(GT24C32C_EEPROM_Ioctl);
EXPORT_SYMBOL(read_moduleID_GT24C32C); // LGE_CHANGE: [2015-11-09] yonghwan.lym@lge.com, Add Module Vendor ID - bug fix
#else
