...
#include <linux/nfc/brcm_gpio_usage.h>
#include <linux/nfc/brcm_eint.h>
...
#define GPIO_NFC_WAKE GPIO_NFC_EINT_PIN  //(117 | 0x80000000)
...
//    mt_eint_set_sens(CUST_EINT_IRQ_NFC_NUM, CUST_EINT_IRQ_NFC_TYPE);
 ...
 //   mt_eint_registration(CUST_EINT_IRQ_NFC_NUM, CUST_EINT_IRQ_NFC_TYPE, bcm2079x_dev_irq_handler, 1);
	...
	if (node) {
		...
		ret =
		    request_irq(nfc_irq, bcm2079x_dev_irq_handler,
		...
		if (ret) {
			...
			pr_info("%s : set EINT finished, nfc_irq=%d", __func__,
				 nfc_irq);
			nfc_irq_count++;
...
static const struct of_device_id bcm2079x_of_match[] = {
...
static struct i2c_driver bcm2079x_driver = {
    ...
    .driver = {
        .owner = THIS_MODULE,
        .name = I2C_ID_NAME,
        .of_match_table = bcm2079x_of_match,
    },
};
...
#if defined (BCM2079X_MTK_PLATFORM)
//    int ret;
//    ret  = platform_driver_register(&bcm2079x_driver);
...
/* change init call api module_init->late_initcall NFC-4016 */ 
late_initcall(bcm2079x_dev_init);
/* change init call api module_init->late_initcall NFC-4016 */ 
