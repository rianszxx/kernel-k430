...
static int SR_default_Get(struct snd_kcontrol *kcontrol,struct snd_ctl_elem_value *ucontrol)
{
...
/* LGE_CHANGE ew0804.kim add gpio control mixer START */
static int Audio_gpio_switch_select_get(struct snd_kcontrol *kcontrol,
					 struct snd_ctl_elem_value *ucontrol)
{
	int FM_radio_TDMB_switch_value = 0;
	pr_warn("%s()  = %ld\n", __func__, ucontrol->value.integer.value[0]);
	FM_radio_TDMB_switch_value = ucontrol->value.integer.value[0];
	if (FM_radio_TDMB_switch_value < 0) {
		pr_err("return -EINVAL\n");
		return -EINVAL;
	}

	return mt_get_gpio_out(FM_radio_TDMB_switch_value);
}


static int Audio_gpio_switch_select_high(struct snd_kcontrol *kcontrol,
					 struct snd_ctl_elem_value *ucontrol)
{
	int FM_radio_TDMB_switch_value = 0;
	pr_warn("%s()  = %ld\n", __func__, ucontrol->value.integer.value[0]);
	...
	mt_set_gpio_out(FM_radio_TDMB_switch_value, 1);

	return 0;
}

static int Audio_gpio_switch_select_low(struct snd_kcontrol *kcontrol,
					 struct snd_ctl_elem_value *ucontrol)
{
	int FM_radio_TDMB_switch_value = 0;
	pr_warn("%s()  = %ld\n", __func__, ucontrol->value.integer.value[0]);
	...
	mt_set_gpio_out(FM_radio_TDMB_switch_value, 0);

	return 0;
}
/* LGE_CHANGE ew0804.kim add gpio control mixer END */
...
/* LGE_CHANGE ew0804.kim add gpio control mixer START */
	SOC_SINGLE_EXT("GPIO_switch_High", SND_SOC_NOPM, 0, 0x80000, 0, Audio_gpio_switch_select_get, Audio_gpio_switch_select_high),
	SOC_SINGLE_EXT("GPIO_switch_Low", SND_SOC_NOPM, 0, 0x80000, 0, Audio_gpio_switch_select_get, Audio_gpio_switch_select_low),
/* LGE_CHANGE ew0804.kim add gpio control mixer END */
};
